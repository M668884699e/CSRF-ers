...
const EditProfileModal = ({ setShowEditProfileModal }) => {
	...
	const [editFirstName, setEditFirstName] = useState(
		currentUserInfo ? currentUserInfo.first_name : currentUserInfo
	);
	const [editLastName, setEditLastName] = useState(
		currentUserInfo ? currentUserInfo.last_name : currentUserInfo
	);
	const [editUserName, setEditUserName] = useState(
		currentUserInfo ? currentUserInfo.display_name : currentUserInfo
	);
	const [editEmail, setEditEmail] = useState(
		currentUserInfo ? currentUserInfo.email : currentUserInfo
	);
	const [editProfileImage, setEditProfileImage] = useState(null);
	const [imageLoading, setImageLoading] = useState(false);
	const [currentPicture, setCurrentPicture] = useState(
		currentUserInfo ? currentUserInfo.profile_image : currentUserInfo
	);
	...
	const onEditProfile = (e) => {
		...
		if (editFirstName.length > 40 || editFirstName.length === 0) {
			errors.push(
				'First name cannot be less than 0 character or more than 40 characters'
			);
		...
		if (editLastName.length > 40 || editLastName.length === 0) {
			errors.push(
				'Last name cannot be less than 0 character or more than 40 characters'
			);
		...
		if (editUserName.length > 40 || editUserName.trim().length === 0) {
			errors.push(
				'Username cannot be less than 0 character or more than 40 characters'
			);
		...
		if (editEmail.length > 255 || editEmail.length === 0) {
			...
			errors.push(
				'Email cannot be less than 0 character or more than 255 characters'
			);
		}
	...
	const handleDeleteUser = () => {
		...
		if (confirmDelete.toLowerCase().trim() === 'delete') {
			...
			return dispatch(userActions.thunkDeleteUser()).then(() => {
			...
			<form onSubmit={onEditProfile} id='epm-form'>
				<section id='epm-form-user-info'>
					...
					<p>
						...
						people to refer to you in Slackers.
					</p>
