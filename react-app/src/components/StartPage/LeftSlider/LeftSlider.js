...
const LeftSlider = () => {
	...
	const selectedMembersList = document.querySelectorAll(
	...
	useEffect(() => {
		...
		const selectedMembers = Object.values(selectedMembersList).map(
			(select) => select.value
		);
	...
	return (
		<section id='sp-ls-section'>
			<main id='sp-ls-section-main'>
				...
				<figure
					className={`sp-ls-section-main-in slsmi-${
						channelNameInputted.length === 0 || !channelInputted
					}`}
				>
					{channelInputted ? (
						<p id='slsmi-true-p'>
							{channelNameInputted.slice(0, 22) +
								(channelNameInputted.length > 23 ? '...' : '')}
						</p>
					) : channelNameInputted.length > 0 ? (
						<figure
							className={`sp-ls-section-main-2in-${
								channelNameInputted.length > 0
							}`}
						>
							{channelNameInputted.slice(0, 22) +
