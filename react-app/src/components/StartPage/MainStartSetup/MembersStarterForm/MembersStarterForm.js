...
const MembersStarterForm = ({ privateChannel }) => {
  ...
  const addMembers = e => {
    ...
    if (inputLength > 0) {
      ...
      Object.values(allTrue).map(currMember => usersIndexes.push(Number(currMember.className.split("-")[3])));
  ...
  const submitChannel = async () => {
    ...
    await dispatch(channelActions.thunkPostNewChannel({
    ...
    // rough draft: go to home page
    return history.push('/');

    // TODO: After getting channel page in chat page, navigate to specific channel
    // navigate to channel page
    // return history.push(`/channels/${newChannel.new_channel.id}`);
  }
  ...
  return (
    <form className="sp-main-section-form">
      ...
      <h2 id="spmsf-h2">
        Who else is on the {channelNameInputted} team?
      </h2>
            ...
            users.map((user, index) => {
              return (
                ...
                >
                  ...
                  <span className="spmsf-members-dn">
                    {user.display_name}
                  </span>
                  <span className="spmsf-members-user-status">
                    <i className="fa-regular fa-circle fa-xs"></i>
                  </span>
                  <span className="spmsf-members-fn">
                    {user.first_name}
                  </span>
                  <span className="spmsf-members-ln">
                    {user.last_name}
                  </span>
                  <span className="spmsf-members-email">
                    {user.email}
                  </span>
                </li>
              );
